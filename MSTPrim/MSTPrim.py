...
class MstPrim:
    ...
    def add_edge_to_mst(self, edge):
        if self.mst_tree.add_edge(edge) == 0:
            raise Exception("Error adding edge")
        self.cost += edge.cost

    def remove_edge_from_mst(self, edge):
        if self.mst_tree.remove_edge(edge) == 0:
            raise Exception("Error removing edge")

        self.cost -= edge.cost

    def is_edge_change_mst(self, edge):
...
def get_edge_that_change_mst(graph, mst):
    edge_found = False
    e = None
    edge_to_be_replaced = None
    ...
    while not edge_found:
        ...
        edge_found, edge_to_be_replaced = mst.is_edge_change_mst(e)

    mst.remove_edge_from_mst(edge_to_be_replaced)
    mst.add_edge_to_mst(e)
    return e


def get_edge_that_dont_change_mst(graph, mst):
    not_found = True
    e = None
    ...
    while not_found:
        ...
        not_found, a = mst.is_edge_change_mst(e)
...
if __name__ == '__main__':
    ...
    edge_that_dont_change = get_edge_that_dont_change_mst(graph, mst)
    write_to_file(file_name, "The edge that didn't changed the MST: ", edge_that_dont_change)

    edge_that_change = get_edge_that_change_mst(graph, mst)
